...
create_schema_directory(ArchiveHandle *AH, const char *tag)
{
	...
	create_directory(AH, "%s/SHELL_TYPES", namespace);
...
_WriteData(ArchiveHandle *AH, const void *data, size_t dLen)
{
	...
	if (dLen > 0 && fwrite(data, 1, dLen, ctx->dataFH) != dLen)
...
_WriteBuf(ArchiveHandle *AH, const void *buf, size_t len)
{
	...
	if (fwrite(buf, 1, len, ctx->dataFH) != len)
		exit_horribly(modulename, "could not write to output file: %s\n",
					  strerror(errno));
}
...
get_object_description(ArchiveHandle *AH, TocEntry *te, PQExpBuffer buf)
{
    ...
    /* use TYPE for shell types */
    if (strcmp(type, "SHELL TYPE") == 0)
        type = "TYPE";
...
add_ownership_information(ArchiveHandle *AH, TocEntry *te)
{
    ...
    if (AH->dopt && AH->dopt->outputNoOwner)
        return;
#endif

	/*
	 * Skip objects that don't have an owner.  Note that while shell types have
	 * an owner, we don't restore it for them because the final CREATE TYPE
	 * would fail if the dump is restored as a different user.
	 */
